...
type mongoExporterWriter struct {
	*baseWriter
...
func newMongoExporterWriter(options *Options, logger io.Writer) writer {
	w := &mongoExporterWriter{
...
func (w *mongoExporterWriter) write(collections []Collection, seed uint64) (err error) {
...
func (w *mongoExporterWriter) generate(coll *Collection) {
...
func (w *mongoExporterWriter) writeToStdout(wg *sync.WaitGroup, coll *Collection, tasks <-chan *rawChunk) {
	defer wg.Done()

	f, err := createFile(fmt.Sprintf("%s.%s.json", coll.DB, coll.Name))
	if err != nil {
		return
	}
	w.out = f
